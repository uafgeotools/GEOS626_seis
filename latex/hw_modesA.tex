...
\item The necessary starting Matlab scripts and data are available from \verb+seismo+ directory on the linux network; type \verb+git pull+ to obtain the latest version of files. The relevant files are:
%
\begin{itemize}
...
%
\begin{enumerate}
\item (0.3) In \refFig{fig:dots}b, plot (by hand) {\em all} the function evaluations $(l, f)$ that you performed.
...
\item (0.4) Compute $F(\bem)$ for all available observations (\refTab{tab:modes_obs}). These observations will be loaded within \verb+spshell_template.m+ if you comment out the last \verb+break+ statement.
...
\item List both $U$ and $U'$ in \refTab{tab:eigfun}.
\end{enumerate}
