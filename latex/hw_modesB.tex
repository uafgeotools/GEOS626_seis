...
\item (1.2) The appearance of overtone branches for \refEq{rODEs} depends on the {\em cut-off frequency of the nth higher mode} \citep[][Eq. 7.8]{AkiRichardsE2}
\footnote{\refEq{wcn} is also listed in a different form in \citet[][p. 92]{SteinWysession}.}
...
\end{equation}
%
where $\beta_c$ is the crustal velocity, $\beta_m$ is the mantle velocity, and $H$ is the thickness of the layer. \refEq{wcn} implies:
%
\begin{itemize}
...
\item (0.2) List the $\cutoff{f} = \cutoff{\omega}/(2\pi)$ for \nrange{1}{10}.
