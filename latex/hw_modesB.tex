...
\item (0.3) Write down the two equations represented by \refEq{rODEs} and the boundary conditions; show explicit $r$ dependence and $n$ dependence (\eg $\omega_n$). For the boundary conditions, do not consider the technicalities in \verb+surf_stress_love.m+; just write down the analogous boundary conditions to those from the toroidal shell problem.

%----------------

\item (1.2) The appearance of overtone branches for \refEq{rODEs} depends on the {\em cut-off frequency of the nth higher mode} 
%
\begin{eqnarray}
...
\item (0.2) List the $\cutoff{f} = \cutoff{\omega}/(2\pi)$ for $\bar{n}=1$--10.
...
\item (0.5) Compute the displacement and stress eigenfunctions (show plots) and values (list in \refTab{tab:k0}) of wavenumber for the fundamental mode, $k_0$ ($n=0$), for periods of 120, 80, 50, 25, 10, and 6 seconds.
...
%
\begin{enumerate}
\item (0.3) Turn in a plot showing the two eigenfunctions, and list your results in \refTab{tab:T3}.
