...
Do not use the words clockwise, counterclockwise, positive, negative.
...
\item (0.5)
\begin{enumerate}
\item (0.3) Sketch your basis vectors on \refFig{fig:block}. Note: You should not use any notation such as $\bxh$, $\byh$, or $\bzh$ throughout this problem. ``Your basis'' is $\brh$ (up), $\bthetah$ (south), $\bphih$ (east).

\item (0.3) What are the coordinates of the following vectors in your basis:
...
\item (1.5) See \refFig{fig:cmt}. In Problem 1, you used the expression $\br = \bU(\rotvec,\rotangB)\br_0$. Here you want to write similar abstract expressions for $\bK$, $\bN$, $\bS$. (What are $\rotvec$, $\rotangB$, and $\br_0$ for each case?)
...
\begin{enumerate}
\item (0.1) List $\bU$.
\item (0.1) Check that $\bU$ is orthonormal.
...
\item (0.6) Draw three points on the beachball in \refFig{fig:block} that correspond to the directions $\{\bu_1,\bu_2,\bu_3\}$. Then do your best to draw them as arrows.
\end{enumerate}
