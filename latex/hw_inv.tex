...
\item (0.2) How many material parameters are needed to describe PREM? Hint: $>$20.
...
\item (0.4) Compute $\vs$ for depths of 1.5~km, 10~km, 20~km, and 50~km; list the values with $>$5 significant digits, so that I can numerically check them. \\
...
\item (0.2) How many observations are represented in \refEq{dT2}? 

How many unknown parameters are represented in \refEq{dT2}? (Assume that each layer is parameterized by a cubic polynomial.)
...
\begin{enumerate}
\item (1.0) Determine a ``forward function'' for a model $\bem = (a,\,b,\,c)$ that inputs a polar angle $\theta_i$ and outputs the point $(x_i,\,y_i)$ on the ellipse described by $\bem$ (\refeq{ellipse}). This function can be thought of as
%
\begin{equation}
